			...
			if (isset($_POST['fdid1'])) {
				if ($id && $_POST['fdid1'] != '') {
					...
					for ($i = 0; $i < $count; $i++) {
						...
						$data1 = array(
							'segmentName' => $_POST['segmentName1'][$i],
							'length' => $_POST['length1'][$i],
							'width' => $_POST['width1'][$i],
							'min' => $_POST['min1'][$i],
							'max' => $_POST['max1'][$i]
						);
				...
				// if($id1){
				for ($j = $i; $j < $count1; $j++) {
					foreach ($_POST['fabricId' . $j] as $fabric) {

					if ($_POST['segmentName'][$j] != '' && $fabric != 0) {
						$data = array(
							'metaId' => $fid,
				...
				for ($i = 0; $i < count($_POST['segmentName']); $i++) {
					foreach ($_POST['fabricId'] as $fabric) {
